...
def get_assets(session):
    """
    Getting all assets that we want to get monitoring data from.

    :param session: The session that has authentication for dynamics
    :return: DataFrame that contains all desired assets and accompanying asset details
    """

    res = use_session(session, API_URL + 'msdyn_customerassets?$select=\
                                            msdyn_name,\
                                            cre90_projectnumber,\
                                            cre90_monitoringplatform,\
                                            cre90_monitoringplatformid,\
                                            msdyn_customerassetid,\
                                            cre90_availableinpegasus,\
                                            cre90_opportunityaddress,\
                                            cre90_timezone')
    res_json = res.json()['value']
    df = pd.DataFrame(res_json)
    df = df[df['cre90_availableinpegasus'] == True]
    df = df.drop(['@odata.etag'], axis=1)
    df = df.rename(columns={'msdyn_name': 'asset_name',
...
def get_asset_proformas(session):
    """
    """
    ...
    df = df.rename(columns={'_cre90_asset_value': 'asset_id',
    ...
    df['field_name'] = df.apply(get_field_names, axis=1)
    df = df.drop(['received_exception', 'net_exception'], axis=1)
...
if __name__ == "__main__":
    ...
    df_assets = get_assets(session)

    df_proformas = get_asset_proformas(session)
    df_proformas.to_csv("test1.csv")

    print(df_proformas)
    print(df_proformas.columns)
